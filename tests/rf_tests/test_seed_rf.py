...
def test_different_seeds_can_diverge():
    ...
    t1 = RandomForest(n_estimators=2, random_state=1)
    t2 = RandomForest(n_estimators=2, random_state=2)
...
def test_random_state_effect():
    ...
    rfs = [
        RandomForest(n_estimators=2, random_state=i, max_features=3, max_height=3)
        for i in range(5)
    ]
    ...
    preds = [t.predict(X) for t in rfs]
